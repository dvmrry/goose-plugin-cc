...
description: Internal guidance for composing prompts for coding, review, diagnosis, and research tasks inside the Goose Claude Code plugin
user-invocable: false
---

# GPT-5.4 Prompting

Use this skill when `goose:goose-task` needs to ask Goose or another GPT-5.4-based workflow for help.

Prompt Goose like an operator, not a collaborator. Keep prompts compact and block-structured with XML tags. State the task, the output contract, the follow-through defaults, and the small set of extra constraints that matter.

Core rules:
- Prefer one clear task per Goose run. Split unrelated asks into separate runs.
- Tell Goose what done looks like. Do not assume it will infer the desired end state.
...
- `<default_follow_through_policy>`: what Goose should do by default instead of asking routine questions.
...
- Write-capable tasks: add `action_safety` so Goose stays narrow and avoids unrelated refactors.
...
- Use `task --resume-last` for follow-up instructions on the same Goose session. Send only the delta instruction instead of restating the whole prompt unless the direction changed materially.
...
- Ask Goose for brief, outcome-based progress updates only when the task is long-running or tool-heavy.
...
3. Decide whether Goose should keep going by default or stop for missing high-risk details.
...
Concrete end-to-end templates live in [references/goose-prompt-recipes.md](references/goose-prompt-recipes.md).
Common failure modes to avoid live in [references/goose-prompt-antipatterns.md](references/goose-prompt-antipatterns.md).
